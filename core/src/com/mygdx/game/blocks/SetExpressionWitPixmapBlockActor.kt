...
class SetExpressionWitPixmapBlockActor: BaseActor() {
    ...
    init {
        ...
        relativeX = drawRightSide(relativeX)
    ...
    private fun drawMiddleSide(relativeX: Int): Int {
        ...
        // Right margin
        pixmap.fillRectangle(margin + triangleMargin + 2 * (triangleSide + 1), 0, cornerRadius + leftMargin, height)
        return margin + triangleMargin + 2 * (triangleSide + 1) + cornerRadius + leftMargin
    }

    private fun drawRightSide(relativeX: Int): Int {
        pixmap.setColor(Color.RED)
        pixmap.fillRectangle(relativeX, 0, width, height)
